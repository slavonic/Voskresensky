...
$FONTFORGE_BIN -lang=ff -c 'Open($1); Generate($2)' sources/Voskresensky.sfd sources/Voskresensky-Regular.ufo
...
FONTINFO=`cat sources/Voskresensky-Regular.ufo/fontinfo.plist`
echo "${FONTINFO/    <key>openTypeOS2Type<\/key>/    <key>openTypeOS2Selection</key>$'\n'    <array>$'\n'      <integer>7</integer>$'\n'    </array>$'\n'    <key>openTypeOS2Type</key>}" > sources/Voskresensky-Regular.ufo/fontinfo.plist
